...
output_path_dutilh = function(a_round = 1, fit = TRUE, subject = useSub, m = model){
  if (fit){
    paste0("modelling/dutilhetal-09/round-", a_round, "/06_output/p", subject, "-", m, ".Rdata")
  } else {
    paste0("modelling/dutilhetal-09/round-", a_round, "/08_model-predictions/p", subject, "-", m, ".Rdata")
  }
}
...
plot_path_dutilh = function(a_round = 1, fit = TRUE, subject = useSub, m = model){
  paste0("modelling/dutilhetal-09/round-", a_round, "/07_plots/")
}
...
load_data_dutilh= function(subject = subj){
  paste0("data/dutilhetal-09/clean/P",subject,".Rdata")
}

# Data structure with relevant file paths for all data sets ---------------
dataset_details <-
  list(
    dataset_number = c(1, 2, 3, 4), # data set number in manuscript 
    dataset_id = c("evans-optim", "evans-normal", "knowles", "dutilh"), # id name 
    load_data_path = c(load_data_optim, load_data_normal, load_data_knowles, load_data_dutilh),
    save_output_path = c(output_path_optim, output_path_normal, output_path_knowles, output_path_dutilh),
    save_plot_paths = c(plot_path_optim, plot_path_normal, plot_path_knowles, plot_path_dutilh),
    save_IC_path = c("data/evansetal-17/derived/optim/", "data/evansetal-17/derived/normal/", "data/knowlesetal-19/derived/", "data/dutilhetal-09/derived/"),
    simulate_fits = c(T,T,T, T), # do you want to simulate the model fits after they are estimated? 
    n_subjects = c(10, 11, 147, 4)
  )

...
full_names <- c("Standard DDM", "a Linear", "a Exponential", "a Delayed Exp", "a Constant Block", "a Delayed Exp Block", "a Exp Trial + Constant Block", "v Linear", "v Exponential", "v Delayed Exponential", "v Constant Block", "v Delayed Exp Block", "v Exp Trial + Constant Block", "a + v Exponential", "a Exp + v Linear", "a Constant Block + v Linear", "a Exp + v Delayed Exp", "v Exp + a Delayed Exp Blocked", "a Delayed Exp + v Linear", "a Constant Blocked + v Delelayed Exp Blocked", "a Delayed Exp + v Delayed Exp Blocked")
```

```{r}
IC_array <- function(models, criterion, n_sub) {
  # set up empty array
  allIC <- as.data.frame(matrix(ncol = length(models), nrow = n_sub))
...
getNBest = function(allIC){
  allIC <- as.data.frame(allIC)
  model_names <- colnames(allIC)
  if(is.null(model_names)){
    model_names <- 1:length(ncol(allIC))
  }
...
```{r}
# allAIC <- IC_array(models, "AIC", n)
# allBIC <- IC_array(models, "BIC", n)
...
load(here("data/knowlesetal-19/derived/allAIC.Rdata"))
load(here("data/knowlesetal-19/derived/allBIC.Rdata"))
```
...
best_BIC <- rankBIC[, 1]
save(best_BIC, file = here("data/knowlesetal-19/derived/best_BIC.Rdata"))
best_AIC <- rankAIC[, 1]
save(best_AIC, file = here("data/knowlesetal-19/derived/best_AIC.Rdata"))
```
...
simpleVBest_AIC <- cbind(simple_AIC, best_no_simple$AIC)
simpleVBest_BIC <- cbind(simple_BIC, best_no_simple$BIC)

# get weighted IC
weighted_simpleVbest_AIC <- weightedICs(simpleVBest_AIC)
colnames(weighted_simpleVbest_AIC) = c("simple", "best alternative")
weighted_simpleVbest_BIC <- weightedICs(cbind(simpleVBest_BIC))
...
colnames(n_param_comparison_BIC) <-
  c("simple","Best 1 parameter model", "Best 2 parameter model")
...
# include simple so we know how many were actually best when comparing against simple. 
tmp_nbest_AIC <- cbind(simple = allAIC[,"simple"], n_param_comparison_AIC)
tmp_nbest_BIC <- cbind(simple = allBIC[,"simple"], n_param_comparison_BIC)

getNBest(tmp_nbest_AIC)
getNBest(tmp_nbest_BIC)


apply(n_param_comparison_weights_AIC, 2, mean)
apply(n_param_comparison_weights_BIC, 2, mean)

plotWeightedICs(n_param_comparison_weights_BIC, main = "BIC", colours = c("purple","blue", "seagreen"))
plotWeightedICs(n_param_comparison_weights_AIC, main = "AIC", colours = c("purple","blue", "seagreen"))
```
...
groups <- c("Standard DDM", "Best block change", "Best trial change")


blockVtrial_AIC <- cbind(allAIC[,"simple"],best_blocked$AIC, best_trial$AIC)
colnames(blockVtrial_AIC) <- groups

weighted_blockVtrial_AIC <- weightedICs(blockVtrial_AIC )


blockVtrial_BIC <- cbind(allBIC[,"simple"],best_blocked$BIC, best_trial$BIC)
colnames(blockVtrial_BIC) <- colnames(blockVtrial_AIC)


weighted_blockVtrial_BIC <- weightedICs(blockVtrial_BIC)


weightedBlockVTrial <- list(AIC = weighted_blockVtrial_AIC, BIC = weighted_blockVtrial_BIC)
save(weightedBlockVTrial, file = here("data/knowlesetal-19/derived/weighted-blocked-v-trial.Rdata"))

getNBest(blockVtrial_AIC)
getNBest(blockVtrial_BIC)

round(apply(weighted_blockVtrial_AIC,2, mean),2)
round(apply(weighted_blockVtrial_BIC,2, mean),2)

plotWeightedICs(weighted_blockVtrial_AIC, main = "AIC", colours = c("purple", "darkgreen", "lightblue"))

plotWeightedICs(weighted_blockVtrial_BIC, main = "BIC", colours = c("purple", "darkgreen", "lightblue"))
...
```{r}
# Assuming 'getNBest' and the subsequent calculations are defined earlier in the code

getNBestAIC <- function(allIC){
...
combined <- cbind(full_names, unname(n_best_BIC), unname(n_best_AIC), 

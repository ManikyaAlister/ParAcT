...
#load(here(paste("data/knowlesetal-19/clean/P",args,".Rdata",sep="")))

for (useSub in subj) { # Run DDM for each subject in nSubj, or a specific subject if running in parallel

  load(here(paste("data/knowlesetal-19/clean/P",useSub,"-Normal-Trial.Rdata",sep="")))
  newSeed=Sys.time()
  ...
  source(here("modelling/knowlesetal-19/round-2/03_priors/03.1.3_a-priors-pow-exp.R"))
  source(here("modelling/knowlesetal-19/round-2/03_priors/03.2.4_v-priors-delay.R"))

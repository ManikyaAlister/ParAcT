...
for (useSub in 1:nSub) {
  load(here(
    paste(
      "modelling/knowlesetal-19/round-2/06_output/P",
      useSub,
      "_a-delayed-power.Rdata",
      sep = ""
    )
  ...
  for (cond in conds) {
    x = c(blah["a.start"], blah["a.asym"], blah["a.rate"], blah["a.delay"],0.5, blah["v"], blah["t0"]) # set parameters
    names(x) = c("a.start", "a.asym", "a.rate","a.delay", "z", "v", "t0")  # set names of the parameters
    ...
    tmp = rdiffusion(
      ...
      v = x["v"],
      t0 = x["t0"],
      z = x["z"] * (x["a.asym"]+x["a.start"]*((x["a.delay"]+1)/(x["a.delay"]+data$Trial^(x["a.rate"]))))
  ...
  save(sim, file = here(
    paste(
      "modelling/knowlesetal-19/round-2/08_model-predictions/P",
      useSub,
      "_a-delayed-power.Rdata",
      sep = ""
    )

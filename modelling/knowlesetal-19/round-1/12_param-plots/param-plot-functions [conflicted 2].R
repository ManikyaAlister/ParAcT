...
thresholdPlot = function(model,nRange){
  ...
  for (i in nRange) {
    ...
    load(here(paste("modelling/knowlesetal-19/round-1/06_output/P", i,"_",model,".Rdata", sep = "")))
...
singleParticipant_a = function(participant,
  ...
  for(model in models){
    load(here(paste("modelling/knowlesetal-19/round-1/06_output/P", participant,"_",model,".Rdata", sep = "")))
    x =  apply(theta, 2, mean)
    if (model == "a-power"){

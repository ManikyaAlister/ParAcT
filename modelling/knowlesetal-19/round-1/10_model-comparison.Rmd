...
only_blocked <- c(both_blocked_models, blocked_models, "v-block-trial-exp", "a-block-trial-exp") # including block + trial models in both only block and only trial
...
only_trial <-  c(models_1p[!models_1p %in% only_blocked & models_1p != "simple"], unique_2p_best[!unique_2p_best %in% mixed_models])
...
blockVtrial_AIC <- cbind(best_blocked$AIC, best_trial$AIC, best_mixed$AIC, allAIC$simple)
colnames(blockVtrial_AIC) <- groups

blockVtrial_BIC <- cbind(best_blocked$BIC, best_trial$BIC, best_mixed$BIC, allBIC$simple)
colnames(blockVtrial_BIC) <- groups
...
save(weighted_blockVtrial_BIC, file = here(paste0(derived_data,"blockVtrial_BIC.Rdata")))
save(weighted_blockVtrial_AIC, file = here(paste0(derived_data,"blockVtrial_AIC.Rdata")))

...
source(file = here("modelling/02_deep-background.R"))
...
for (useSub in 1:nSub) { #Run DDM for each subject in n Subjects
  ...
  savefile=here(paste("modelling/06_output/P",useSub,"Simple_Model.Rdata",sep=""))
  
  source(here("modelling/03_background.R"))
  source(here("modelling/04_iterative-process.R"))
...
for(useSub in 1:nSub) {
  ...
  load(here(paste("modelling/06_output/P",useSub,"Simple_Model.Rdata", sep = ""))) #Loads through the datasets of each participant in nSub
  ...
  for (cond in conds) { # Loops through each cue condition (congruent and incongruent)
    currParams=c(blah["a.start"],blah["a.asym"],blah["a.rate"],0.5,blah["v"],blah["t0"]) # Sets the value of parameters. 
    names(currParams)=c("a.start","a.asym","a.rate","v",
                        "t0") # Sets the names of the parameters

...
for (useSub in 1:nSub) {
  ...
  for (cond in conds) {
    x = c(blah["a.start"], blah["a.asym"], blah["a.rate"], 0.5, blah["v.b"], blah["v.c"], blah["t0"]) # set parameters
    names(x) = c("a.start", "a.asym", "a.rate", "z", "v.b","v.c", "t0")  # set names of the parameters
    ...
    tmp = rdiffusion(
      ...
      t0 = x["t0"],
      z = x["z"] * (x["a.asym"]+x["a.start"]*data$Trial^(-x["a.rate"]))

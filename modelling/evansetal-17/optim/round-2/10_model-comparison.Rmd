...
best_BIC <- rankBIC[, 1]
save(best_BIC, file = here("data/evansetal-17/derived/optim/best_BIC.Rdata"))
best_AIC <- rankAIC[, 1]
save(best_AIC, file = here("data/evansetal-17/derived/optim/best_AIC.Rdata"))
```
...
blocked_models <- models[grep("blocked|step", models)]
blocked_models <- blocked_models[both_blocked_models!=blocked_models]

trial_models <- models[!models %in% blocked_models & models != "simple" & models != both_blocked_models] 
...
best_mixed <- best_IC_from_models(blocked_models)
best_trial <- best_IC_from_models(trial_models)
best_blocked<- best_IC_from_models(both_blocked_models)

groups <- c("Best block change", "Best trial change", "Best Mix")
weighted_blockVtrial_AIC <- weightedICs(cbind(best_blocked$AIC, best_trial$AIC, best_mixed$AIC))
colnames(weighted_blockVtrial_AIC) <- groups

weighted_blockVtrial_BIC <- weightedICs(cbind(best_blocked$BIC, best_trial$BIC, best_mixed$BIC))
...
plotWeightedICs(weighted_blockVtrial_AIC, main = "AIC", colours = c("red", "blue", "orange"))
plotWeightedICs(weighted_blockVtrial_BIC, main = "BIC",colours = c("red", "blue", "orange"))
...
count_blockedVtrial_AIC <- apply(weighted_blockVtrial_AIC, 1, which.max)
table(count_blockedVtrial_AIC)
count_blockedVtrial <- c(block = sum(count_blockedVtrial == 1), trial = sum(count_blockedVtrial == 2), both = sum(count_blockedVtrial == 3))
count_blockedVtrial

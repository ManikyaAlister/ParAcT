...
thresholdPlot = function(model,nRange, round = 1, complex = FALSE, mean = FALSE, title = "", subtitle = paste0("n = ", length(nRange))){
  ...
  for (subject in nRange) {
    ...
    load(here(paste("modelling/evansetal-17/normal/round-",round,"/06_output/P", subject,"_",model,".Rdata", sep = "")))
    ...
    load(here(paste("modelling/evansetal-17/normal/round-1/06_output/P", subject,"_a-blocked-complex.Rdata", sep = "")))
...
getDriftChange = function(model, parameters, data){
  ...
  } else if (model == "v-blocked-simple") {
    d = x["step"] * (data$Block-1)
    drift = x["v"]+d
  }
...
driftPlot = function(model,nRange, round = 1, complex = FALSE, mean = FALSE, title = "",subtitle = paste0("n = ", length(nRange))){
  ...
  for (i in nRange) {
    ...
    load(here(paste("modelling/evansetal-17/normal/round-",round,"/06_output/P", i,"_",model,".Rdata", sep = "")))
    ...
    # load complex block model that estimates a different parameter each block 
    load(here(paste("modelling/evansetal-17/normal/round-1/06_output/P", i,"_v-blocked-complex.Rdata", sep = "")))
    complexParams = apply(theta, 2, mean)
    complexAll = rbind(complexParams, complexAll)
  }
...
#   for (i in nRange) {
#     
#     load(here(paste("modelling/evansetal-17/normal/round-2/06_output/P", i,"_",model,".Rdata", sep = "")))
#     
#     trials = 1:length(data$Trial)
#     x =  apply(theta, 2, mean)
#     
...
singleParticipant_a = function(participant,
  ...
  for(model in models){
    load(here(paste("modelling/evansetal-17/normal/round-",round,"/06_output/P", participant,"_",model,".Rdata", sep = "")))
    x =  apply(theta, 2, mean)
    ...
    if(complex){
      # load complex block model that estimates a different parameter each block 
      load(here(paste("modelling/evansetal-17/normal/round-1/06_output/P", participant,"_a-blocked-complex.Rdata", sep = "")))
...
singleParticipant_v = function(participant,
  ...
  for(model in models){
    ...
    if(complex){
      # load complex block model that estimates a different parameter each block 
      load(here(paste("modelling/evansetal-17/normal/round-1/06_output/P", participant,"_v-blocked-complex.Rdata", sep = "")))

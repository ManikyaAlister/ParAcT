...
for (useSub in 1:nSub) {
  ...
  load(here(
    paste("data/evansetal-18/round-1/clean/P",useSub, ".Rdata", sep = "")
  ))
  newSeed = Sys.time()
  ...
  log.dens.like = function (x, data, par.names) {
    ...
    for (block in blocks) {
      ...
      s = 1
      tmp = ddiffusion(
        rt = data$Time[data$Block == blocks],
        response = data$Resp[data$Block == blocks],
        z = z * a,
        a = a,
        v = v,
        t0 = t0 - (st0 / 2),
  ...
  source(here("modelling/evansetal-18/round-1/03_priors/03.1.1_priors-simple.R"))

...
subj = commandArgs(trailingOnly = TRUE) # if running in parallel, this will be a vector of length 1 with the subject number

####################################
#### Exponential Threshold Model ###
####################################

for (useSub in 1:10) { # Run DDM for each subject in nSubj, or a specific subject if running in parallel

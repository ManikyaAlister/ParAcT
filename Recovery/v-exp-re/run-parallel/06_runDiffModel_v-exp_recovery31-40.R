...
model = "v-exp-re" 
nSub = 40 # number of subjects to run 

####################################
#### Exponential Threshold Model ###
####################################

for (useSub in 31:nSub) { # Run DDM for each subject in n Subjects
  ...
  savefile=here(paste("Recovery/v-exp-re/Fits_recovery/P",useSub,"_",model,".Rdata",sep=""))

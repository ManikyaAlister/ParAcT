...
thresholdPlot = function(model,nRange, round = 1, complex = FALSE, mean = FALSE){
  ...
  for (i in nRange) {
    ...
    load(here(paste("modelling/evansetal-17/normal/round-1/06_output/P", i,"_a-blocked-complex.Rdata", sep = "")))
...
driftPlot = function(model,nRange, round = 1, complex = FALSE, mean = FALSE){
  ...
  for (i in nRange) {
    ...
    load(here(paste("Recovery/model-recovery/v-exp-generated/fits/P", i,"_",model,".Rdata", sep = "")))
    ...
    # load complex block model that estimates a different parameter each block 
    load(here(paste("Recovery/model-recovery/v-exp-generated/fits/P", i,"_complex-blocked.Rdata", sep = "")))
    complexParams = apply(theta, 2, mean)
    complexAll = rbind(complexParams, complexAll)
  }
...
singleParticipant_a = function(participant,
  ...
  for(model in models){
    load(here(paste("modelling/evansetal-17/normal/round-2/06_output/P", participant,"_",model,".Rdata", sep = "")))
    x =  apply(theta, 2, mean)
    if (model == "a-power"){

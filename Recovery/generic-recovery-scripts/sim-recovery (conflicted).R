...
for (i in 1:nrow(use.LHS)) {
  ...
  genParams=array(NA,c(nParams,length(stims)),dimnames=list(theta.names,stims))
  ...
  for (stim in stims) {
    ...
    genParams[,paste(stim)] = as.numeric(use.LHS[i,theta.names])
  
    # # z should be different depending on the stimulus, so this updates it. 
    # if (stim==stims[1]) {
    #   genParams["z",paste(stim)]=as.numeric(use.LHS[i,"z"])
    # } else if (stim==stims[2]) {
    #   genParams["z",paste(stim)]=(1-as.numeric(use.LHS[i,"z"]))
    # } else {
    ...
    # Actually simulate
    tmp=simulate.DIFF(N=n_trials,params=genParams[,paste(stim)],maxCounter=50000,stepSize=0.001,     ##### NJE: Changed "cond" to "stim" here
  ...
  # Save sim data
  #save(file=paste("Recovery/",model,"/Datasets/RECOVERY_DATA-DIFF_LHS-",i,".Rdata",sep=""),data,genParams,stims)     ##### NJE: Changed "conds" to "stims" here

...
simulate.DIFF <- function(N, params, maxCounter, stepSize, varyV, varyA, varyZ, varyT0, use.table, n.table.options) {
  ...
  if (varyV) {
    v <- (params["v.asym"] + params["v.start"]) - params["v.start"] * ((params["v.delay"] + 1) / (params["v.delay"] + exp(params["v.rate"] * (1:N))))
  } else {
    v <- rep(params["v"], N)
  }

  if (varyA) {
    a <- params["a.asym"] + (params["a.start"] * ((params["a.delay"] + 1) / (params["a.delay"] + exp(params["a.rate"] * (1:N)))))
  } else {
    a <- rep(params["a"], N)
  }

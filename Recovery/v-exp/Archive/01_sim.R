...
for (i in 1:nrow(use.LHS)) {
  ...
  for (cond in conds) {
    ...
    genParams[,paste(cond)] = c(tmp,
                                ...
                                as.numeric(use.LHS[i,"t0"]) + (cond*as.numeric(use.LHS[i,"t0.change"])),
                                as.numeric(use.LHS[i,"v.start"]) + (cond*as.numeric(use.LHS[i,"v.start"])),
                                as.numeric(use.LHS[i,"v.asym"]) + (cond*as.numeric(use.LHS[i,"v.asym"])),
                                as.numeric(use.LHS[i,"v.rate"]) + (cond*as.numeric(use.LHS[i,"v.rate"])),
                                1,0,0,0)
    
    # Actually simulate
    tmp=simulate.DIFF(N=100,params=genParams[,paste(cond)],maxCounter=10000,stepSize=0.001,

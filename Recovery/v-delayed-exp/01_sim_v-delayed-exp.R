...
colnames(use.LHS) <- c(
  "v.start", "v.asym", "v.rate", "v.delay", # v. center/change params? Maybe not because we're not looking at diffs across conditions
  "a", "t0", "z"
)
...
for (i in 1:nrow(use.LHS)) {
  ...
  genParams <- array(NA, c(nParams + 4, length(conds)), dimnames = list(c("z", "a", "ter", "v.start", "v.asym", "v.rate", "v.delay", "stoch.s", "sz", "sv", "ster"), conds))
